...
class  ADFO_loader_documentation {
	...
	public function fn_get_documentation() {
		require_once(__DIR__.'/af-function-documentation.php');
		...
		$get_page = isset($_REQUEST['get_page']) ?  sanitize_text_field($_REQUEST['get_page']) : 0;
		$search = isset($_REQUEST['get_search']) ? strtolower( sanitize_text_field($_REQUEST['get_search'])) : '';
		...
		if ($get_page != "" ) {
			...
			if (substr($get_page, 0, 8) == "admin_form_docs") {
				$get_page = "admin_form_docs-menu.php";
			}
			if ( is_file($dir_scan . "/" . $get_page) ) {
				...
				ob_start();
				?>
				<div class="af-content-margin dbp-sidebar-breadcrumb">
					<?php
