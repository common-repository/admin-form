...
class  ADFO_html_table {
	...
	public function template_render($table_model) {
		...
		if ($table_model->last_error !== false) {
			...
			?><div class="dbp-alert-gray"><?php echo sprintf(__('MySQL returned %s rows. (Query took %s seconds.)', 'admin_form'), $table_model->effected_row,  $table_model->time_of_query ); ?></div><?php 
		}
	...
	public function render($items, $sorting = false, $searching = false) {
		...
		// array_thead è compilata su af-functions-item-setting.php > execute_list_settings
				...
				foreach ($array_thead as $key => $value) {
					...
					if ($value->type == "CHECKBOX"  ) {
						...
						<th class="dbp-table-th dbp-th-dim-<?php echo strtolower($value->type). $value->width; ?>">
							<?php 
							$dropdwon_html = "";
							$curr_searching = $searching;
							if ($value->dropdown) {
								$lite = ($value->is_multiple);
								
								ob_start();
								$this->dropdown($value->field_key, $value->original_field_name, $value->name_column, $value->type, $value->original_table, $row_sorting, $curr_searching, $lite);
		...
		foreach ($items as $item) : ?>
			...
			<tr id="<?php echo $id_base.'_'.$count_row; ?>">
				...
				foreach ($array_thead as $key=>$setting) { 
					...
					$align = "";
					if (isset($setting->align) && $setting->align != "") {
						$align = " dbp-td-".$setting->align;
					}
					...
					?><td class="dbp-table-td<?php echo $setting->width.$align.' '.$formatting_class; ?>"><div class="btn-div-td btn-div-td-<?php echo $css_name; ?>" data-dbp_rif_value="<?php echo esc_attr($key); ?>"><?php echo $item->$key; ?></div></td> <?php
	...
	static function icons($alias_column, $original_field_name, $sort, $filter, $name_column) {
		...
		if (is_array($filter)) {
			foreach ($filter as $f) {
				$between =  ADFO_fn::is_correct_between_value($f['value'], $f['op']);
	 ...
	 * since 2.0.0
	 * @param bool $lite se true disegna una versione limitata del dropdown 
	 * 
	 * @return String  l'html dell'ordinamento delle colonne
	 */
	static function dropdown($alias_column, $original_field_name, $name_column, $type, $original_table, $sort, $filter, $lite = false) {
		if ($alias_column == "") return "";
		if ($sort !== false)  {
		...
		if ($filter !== false)  {	
			$symple_type = $type;
			$name_column = ADFO_fn::clean_string($name_column);
			$def_op = "=";
			$def_input_value = "";
			list($html_select_array, $def_op) =  ADFO_fn::get_array_for_select_in_drowdown_filter($type);
			$default_value = $def_input_value_2 = "";
			
			if (@is_array($filter)) {
				foreach ($filter as $f) {
					if (is_array($f['value'])) continue;
		...
		if ($lite) {
			require (ADFO_DIR."/admin/partials/af-partial-dropdown-lite.php");
		} else if ($sort !== false || $filter !== false)  {	
			require (ADFO_DIR."/admin/partials/af-partial-dropdown.php");
		}
	}
}