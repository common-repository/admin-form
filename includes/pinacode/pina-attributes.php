 ...
 * [% val]
 */
if (!function_exists('pinacode_attr_fn_val_all')) {
	function pinacode_attr_fn_val_all($gvalue, $param, $shortcode_obj) {
		$new_val = '';
		if (substr(strtolower($shortcode_obj['shortcode']),0, 5) == 'data.') {
			$new_val = 'data.____af__'.substr($shortcode_obj['shortcode'], 5);
		}
		if (substr(strtolower($shortcode_obj['shortcode']),0, 5) == 'item.') {
			$new_val = 'item.____af__'.substr($shortcode_obj['shortcode'], 5);
		}
		if ($new_val != '' && PinaCode::has_var($new_val)) {
			$gvalue = PinaCode::get_var($new_val);
		}
		return $gvalue;
	}
}
pinacode_set_attribute('val', 'pinacode_attr_fn_val');
 ...
 * [% timenum-to-timestr]
 */
if (!function_exists('pinacode_attr_fn_timenum_to_timestr')) {
	function pinacode_attr_fn_timenum_to_timestr($gvalue, $param, $shortcode_obj) {
		$gvalue = (int)$gvalue;
		if ($gvalue >= 0 && $gvalue <= (86400)) {
			$hour = floor($gvalue  / 3600);
			$min = floor(($gvalue  - ($hour * 3600)) / 60) ;
			$gvalue = str_pad($hour,2,'0', STR_PAD_LEFT).':'.str_pad($min,2,'0', STR_PAD_LEFT);
		} else {
			$gvalue = "00:00";
		}
		return $gvalue;
...
pinacode_set_attribute('timenum-to-timestr', 'pinacode_attr_fn_timenum_to_timestr');

/**
 * [% timestr-to-timenum]
 */
if (!function_exists('pinacode_attr_fn_timestr_to_timenum')) {
	function pinacode_attr_fn_timestr_to_timenum($gvalue, $param, $shortcode_obj) {
		$gvalue = explode(":", $gvalue);
		if (count($gvalue) == 2 && is_numeric($gvalue[0]) && is_numeric($gvalue[1])) {
			$gvalue = ($gvalue[0] * 3600) + ($gvalue[1] * 60);
		} else {
			$gvalue = 0;
		}
		return $gvalue;
...
pinacode_set_attribute('timestr-to-timenum', 'pinacode_attr_fn_timestr_to_timenum');

/**
 * [% time-to-hour]
 */
if (!function_exists('pinacode_attr_fn_time_to_hour')) {
	function pinacode_attr_fn_time_to_hour($gvalue, $param, $shortcode_obj) {
		$value = explode(":", $gvalue);
		...
		$gvalue = (int)$value;
		return floor($gvalue  / 3600);
	}
}
pinacode_set_attribute('time-to-hour', 'pinacode_attr_fn_time_to_hour');
...
/**
 * [% decode_ids] ritorna gli id di un record a partire dalla stringa compressa con get_ids_value_from_list
 */
if (!function_exists('pinacode_attr_fn_decode_ids')) {
	function pinacode_attr_fn_decode_ids($gvalue, $param, $shortcode_obj) {
		if (is_array($gvalue) || is_object($gvalue)) {
			...
			return ADFO_fn::ids_url_decode($gvalue);
		}
	}
}
 ...
 * [% json]
 */
if (!function_exists('pinacode_attr_fn_json')) {
	function pinacode_attr_fn_json($gvalue, $param, $shortcode_obj) {
		if (is_string($gvalue)) {
			$temp_gvalue = json_decode($gvalue);
			if (json_last_error() != JSON_ERROR_NONE) {
				$temp_gvalue = pinacode_attr_fn_val_all($gvalue, $param, $shortcode_obj);
				$temp_gvalue = json_decode($temp_gvalue);
				if (json_last_error() != JSON_ERROR_NONE) {
					PcErrors::set('JSON PARSE - '.get_json_error_decode().' IN <b>'.substr($gvalue,0,15)."</b>", substr($gvalue,0, 30), -1, 'warning');
				} else {
					$gvalue = $temp_gvalue;
				}
			} else {
...
pinacode_set_attribute('json', 'pinacode_attr_fn_json');
