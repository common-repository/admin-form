	...
	**/
	...
	static function execute($short_code_transformed) {
		...
		if (isset($short_code_transformed['shortcode']) && isset($short_code_transformed['attributes'])) {
			$short_code_transformed['shortcode'] = trim($short_code_transformed['shortcode']);
			$split_shortcode = explode(".", $short_code_transformed['shortcode']);
			$first = array_shift($split_shortcode);
			if (count($split_shortcode) > 0) {
				$short_code_transformed['shortcode'] = strtolower($first) . "." . implode(".", $split_shortcode);
			} else {
				$short_code_transformed['shortcode'] = strtolower($first);
			}
			//$short_code_transformed['shortcode'] = strtolower($first) . "." . implode(".", $split_shortcode);

			$shortcode = $short_code_transformed['shortcode'];
			$function_name = "";
