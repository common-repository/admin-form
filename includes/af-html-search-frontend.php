...
class  ADFO_search_form {
	...
	function classic_search_link($prefix_request, $base_link = false, $color = 'blue') {
		$uniqid = 'tmp_' . ADFO_fn::get_uniqid();
        if ($base_link == false) {
		...
		<div class="dbp-search-row">
			<input id="<?php echo esc_attr($uniqid); ?>" type="text" name="<?php echo esc_attr($prefix_request); ?>_search" value="<?php echo ADFO_fn::esc_request($prefix_request.'_search'); ?>" class="dbp-search-input">
			<div class="dbp-search-button dbp-search-button-<?php echo $color; ?>" onclick="window.location='<?php echo esc_url($complete_link); ?>='+encodeURIComponent(document.getElementById('<?php echo esc_attr($uniqid); ?>').value);">Search</div>
		</div>
		<?php
	}
    ...
    public function classic_search_post($prefix_request, $label = 'Search', $color = 'blue',  $btn = true) {
        $req_search =  isset($_REQUEST[$prefix_request.'_search']) ? sanitize_text_field($_REQUEST[$prefix_request.'_search']) : '';
        ?>
        <div class="dbp-search-row">
                <input class="dbp-search-input js-dbp-search-input" type="search" name="<?php echo esc_attr($prefix_request); ?>_search" value="<?php echo esc_attr($req_search); ?>">
            <?php // TODO il submit deve essere passato ad una funzione js che pulisce il campo page e order? ?>
            <?php if ($btn) : ?>
                <div class="dbp-search-button dbp-search-button-<?php echo esc_attr($color); ?>" onclick="dbp_submit_simple_search(this)"><?php echo esc_attr($label); ?></div>
            
                <?php if ($req_search  != "") : ?>
                    <div class="dbp-search-button dbp-search-button-<?php echo esc_attr($color); ?>" onclick="dbp_submit_clean_simple_search(this)"><?php _e('Clean', 'admin_form'); ?></div>
                <?php endif; ?>
            <?php endif; ?>
    ...
    public function field_one_input_form($field_name_request, $label) {
        ...
        $field_name_request = isset($_REQUEST[$field_name_request]) ? sanitize_text_field($_REQUEST[$field_name_request]) : '';
        ?>
        <input class="dbp-search-input js-dbp-search-input" type="search" name="<?php echo esc_attr($field_name_request); ?>" value="<?php echo esc_attr($field_name_request); ?>">
        <?php if ($label != "") : ?>
            </label>
        <?php endif; 
    }
    ...
    public function one_field_input_form($link, $method, $field_name, $btn_text, $div_row_attributes, $label) {
        ...
        <div <?php echo $div_row_attributes; ?>>
            ...
            <label><span class="dbp-search-label"><?php echo $label; ?></span>	
            <?php endif; ?>
                <input class="dbp-search-input" type="text" name="<?php echo esc_attr($field_name); ?>" value="<?php echo esc_attr($field_name ); ?>">
            <?php if ($label != "") : ?>
                </label>
            <?php endif; ?>
            <?php
        if ($form_open) {
            ...
            ?>
            <button type="submit" class="dbp-search-button dbp-search-button-parent-attr"><?php echo esc_attr(_e($btn_text, "admin_form")); ?></button>
             </div>
             </form><?php
        } else {
            ?>
     ...
     public function open_form($link, $method) {
        ...
        self::$form_id = 'dbp_' . ADFO_fn::get_uniqid();
        self::$form_open = true;
    ...
    public function close_form($btn_text, $div_row_attributes = "", $color="blue") {
        ...
        if ($btn_text != false) {
            ?>
                <div <?php echo $div_row_attributes; ?>>		
                <button type="submit" class="dbp-search-button dbp-search-button-<?php echo $color; ?>"><?php echo esc_attr(_e($btn_text, "admin_form")); ?></button>
                </div>
            <?php
        }
        echo '</form>';
    ...
    public function add_params_list_per_post($post_id, $array_exclude) {
        ...
        if (self::$form_open && is_array($_REQUEST)) {
            ...
            foreach ($request as $key=>$value) {
                if ($key == 'dbp'.$post_id."_sort") continue;
    ...
    public function pagination_link($link, $total_items, $limit, $post_id, $pagination_style, $color) {
		...
		if (isset($_REQUEST[$prefix."_page"])) {
			$curr_page = absint($_REQUEST[$prefix."_page"]);
		}
        ...
        $total_items_name = apply_filters('adfo_frontend_total_items', __(sprintf('%s items', $total_items), 'admin_form'), $post_id, $total_items,  $limit, $curr_page, $pages );
		?>
    ...
    public function pagination_form($prefix, $total_items, $limit, $post_id, $pagination_style, $color, $method) {
		...
		$uniqid = 'dbp_' . ADFO_fn::get_uniqid();
