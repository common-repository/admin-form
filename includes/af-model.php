...
class  ADFO_model {
     ...
     * Viene compilato solo da ADFO_fn::items_add_action
     */
     ...
     * @param Mixed $table_name String | Array [table_alias:table] 
     */
    public function __construct($table_name = "") {
        ...
        if (is_array($table_name)) {
            ...
            foreach ($table_name as $key => $val) {
                $select[] = '`' . ADFO_fn::sanitize_key($key)."`.*";
                $from[] = '`' . ADFO_fn::sanitize_key($val) . '` `' . ADFO_fn::sanitize_key($key) . '`';
            }
            $table_name = reset($table_name); 
            $this->prepare("SELECT ".implode(",", $select). " FROM ".implode(",", $from));
        } else {
    ...
    public function list_add_limit($limit_start, $limit) {
        ...
        if($this->sql_type() == "select") {
            $this->current_query = ADFO_fn::all_trim($this->current_query);
    ...
    public function get_partial_query_where($split = false) {
        ...
        if ($split) {
            ...
            $list_of_parts = ADFO_fn::multi_explode($neddles, $from);
            foreach ($list_of_parts as $single_from) {
                list($pos, $join) = ADFO_fn::find_first($neddles, $single_from);
    ...
    public function removes_column_from_where_sql($column) {
        ...
        if ($where_string  !== '') {
            ...
            $where_array = ADFO_fn::multi_explode([' AND ',' OR ', 'AND(',' OR(' , ')AND ',')OR '],  $this->ulitities_marks->restore($where_string), true);
            $new_where = [];
            $skip = false;
            ...
            foreach ($list_of_parts as $single_from) {
               // print "<p>".$single_from."</p>";
                ...
                if (count($from_ris_2) == 2) {
                    ...
                    $form_2 = ADFO_fn::all_trim($this->ulitities_marks->restore(array_shift($from_ris_2)));
                } else {
                    $left = str_ireplace([' as ', '  '], ' ', ADFO_fn::all_trim($single));
                    $form_2 = '';
                }
                $left_2 = explode(" ", $left);
                ...
                if (count ($left_2) == 1) {
                    $table = ADFO_fn::all_trim(str_replace("`","", $this->ulitities_marks->restore(reset($left_2))));
                    $table_alias = trim(str_replace("`","", $this->ulitities_marks->restore(reset($left_2))));
                    $from_split[] = [$table, $table_alias, '',  ADFO_fn::all_trim($this->ulitities_marks->restore($single_from))] ;
                }
                if (count ($left_2) == 2) {
                    $table = ADFO_fn::all_trim(str_replace("`","", $this->ulitities_marks->restore(array_shift($left_2))));
                    $table_alias = trim(str_replace("`","", $this->ulitities_marks->restore(array_shift($left_2))));
                    $from_split[] = [$table, $table_alias, $form_2,   ADFO_fn::all_trim($this->ulitities_marks->restore($single_from))];
                }
        ...
        if ($this->sql_type() == "select") {
            ...
            if ($limit > 0) {
                ...
                if (count($limit_temp) == 1) {
                    $limit_start = 0;
                    $limit_end = intval(ADFO_fn::all_trim(array_shift($limit_temp)));
                }
                if (count($limit_temp) == 2) {
                    $limit_start = intval(ADFO_fn::all_trim(array_shift($limit_temp)));
                    $limit_end = intval(ADFO_fn::all_trim(array_shift($limit_temp)));
                }
            } 
        }
    ...
    /**
     * Estrae la parte di query dell'order by
     * @param Boolean $split se dividerlo in gruppi oppure no. SE diviso in gruppi ritorna 
     * [[table, field, as, all_part], ...]
     * @return Array  [limit_start, limit_end] [0,0] se il limite non è impostato
     */
    public function get_partial_query_order() {
        if ($this->sql_type() == "select") {
            $current_query = $this->current_query;
            $order = stripos($current_query, 'order by ');
           
            if ($order > 0) {
                $order_string = str_replace(";", "", substr($current_query, $order + 9));
                
                $limit = trim(stripos($order_string, ' limit'));
                if ($limit > 0) {
                    $order_string = substr($order_string, 0, $limit);
                }
                $order_string = $this->ulitities_marks->restore($order_string);
                $order_array = explode(" ", str_replace(["\n", "\t"], " ", $order_string));
                $order_array = array_filter($order_array);
                if (count ($order_array) >= 2) {
                    $ordering = array_pop($order_array);
                    return [implode(" ", $order_array), $ordering];
                }
            } 
        }
        return [];
    ...
    public function list_change_from($from) {
        ...
        if ($first_occurrance === false) return "";
        list($found_string, $pos) = $this->strpos_array($this->current_query, ['where ','order ','group ','limit ','having '],  $first_occurrance);
        $select =  substr($this->current_query, 0, $first_occurrance);
        while(substr($select,-1) == " " || substr($select,-1) == "\t" || substr($select,-1) == "\n"  || substr($select,-1) == "\r" ) {
            $select = substr($select,0,-1);
        }
        if ($found_string != "") {
            $this->current_query = $select . " FROM " . $from . " " . substr($this->current_query, $pos);
        } else {
            $this->current_query = $select . " FROM " . $from;
        }
    }
    ...
    public function list_add_select($add_select) {
        ...
        $select = ADFO_fn::all_trim(substr($current_query, $select_string , $from - $select_string));
    ...
    public function list_add_order($field, $order) {
        if($this->sql_type() == "select") {
            ...
            $sql = ADFO_fn::all_trim($sql);
            if ($field == "RANDOM") {
                $this->current_query = $sql." ORDER BY RAND()";
            } else {
                $this->current_query = $sql." ORDER BY ".ADFO_fn::sanitize_key($field)." ".$order;
            }
            if ($limit_sql != "") {
     ...
     */
    public function list_add_where($filter, $conjunction = "AND", $add_in_filter = true) {
        if($this->sql_type() == "select" && is_array($filter) && count($filter) > 0) {
            ...
            if ($where_filter != "") {
                if ($conjunction == "AND" && $add_in_filter) {
                    $this->filter = $filter;
                }
            } else {
                return "";
    ...
    public function get_list($check_same_column = true, $force_select = true) {
        ...
        if ($this->sql_type() == "select" || $this->sql_type() == "show") {
                ...
                if (is_array($first_array)) {
                    ...
                    foreach ($list_last_get_col_info as $key => $schema ) {
                        if (isset($schema->name)) {
                            ...
                            $schema->name = (is_numeric($schema->name)) ? (int)$schema->name : (string)$schema->name;
            ...
            if ($draft_is_ok && $close_is_ok) {
                ...
                if (!$draft_is_ok) {
                    $this->last_error = __('This type of query is not allowed because the table is in PUBLISH state', 'admin_form');
                } else {
                    $this->last_error = __('This type of query is not allowed because the table is in CLOSE state', 'admin_form');
                }
                $this->effected_row = 0;
                $result = [['effected_row' =>['schema'=>(object)['name'=>'effected_row','orgname'=>'','orgtable'=>'','table'=>'','type'=>'','db'=>'']]], ['effected_row'=>0]];
            }
     ...
     * @deprecated Passato a includes\af-model-structure.php
 	 */
      public function get_structure() {
        ...
        $this->last_error = false;
        $this->effected_row = -1;
        $sql = 'SHOW COLUMNS FROM `'. ADFO_fn::sanitize_key($this->table_name).'`';
        $result = $wpdb->get_results($sql);
    ...
    public function remove_order() {
        if($this->sql_type() == "select") {
            ...
            $this->current_query =  ADFO_fn::all_trim($sql);
            if ($limit_sql != "") {
               $this->current_query = $this->current_query . " " . $limit_sql;
            }
        }
        ...
        if (is_array($this->items) && count($this->items) > 0) {
            ...
            foreach ($table_header as $k=>$th) {
                if (isset($th->table) && $th->original_table != "" && !isset($tables[$th->original_table])) {
                    $tables[$th->original_table] = ADFO_fn::get_dbp_option_table($th->original_table);
                    if ($table_status == '') {
     ...
     */
    public function update_items_with_setting($post = false, $htmlentities = true, $text_length = 0, $export = false) {
        $dbp_id = 0;
        $post_content = ADFO_functions_list::convert_post_content_to_list_params();
        if ($post != false) {
            $post_content = $post->post_content;
            $dbp_id = $post->ID;
        } 
        if (is_array($post_content) && isset($post_content['list_setting'])) {
            // trasformo le colonne delle liste con i valori dei post
            $setting_custom_list =  ADFO_functions_list::get_list_structure_config($this->items, $post_content['list_setting']);
        } else {
        ...
        $list_general_setting['export'] = $export;
        if ($text_length != 0) {
    ...
    public function distinct($column, $filter = "") {
        ...
        if ($pri == "") {
            ...
            if (strpos($column, '`.`') !== false) {
                ...
                $pri_alias = '`'.ADFO_fn::sanitize_key($table_alias).'`.`'.ADFO_fn::sanitize_key($pri).'`';
            } else {
                $pri_alias = '`'.ADFO_fn::sanitize_key($this->table_name).'`.`'.ADFO_fn::sanitize_key($pri).'`';
            }
            $new_fragment = ' '.ADFO_fn::sanitize_key($column).' AS c, ' . $pri_alias . ' as p, count('. $pri_alias .') as n ';
        }
    ...
    public function get_pirmaries($alias = false) {
        ...
        if (!$alias) {
            foreach ($tables as $table) {
                ...
                $pri = ADFO_fn::get_primary_key($orgtable);
                if ($pri != "") {
     ...
     * @return Array solo le nuove primary ID
     */
        ...
        foreach ( $field_group as $group) {
            ...
            if (isset($all_pri_ids[$group['table']])) {
                ...
                if (!$exist_pri) {
                    ...
                    $all_primaries[$fields->table] = $all_pri_ids[$group['table']];
                    $add_primaries[$fields->table] = $all_pri_ids[$group['table']];
                }
            }
        }
                    ...
                    if (count($value_temp) == 2) {
                        ...
                        if ($base_table == $table) {
                            if (strpos($value_field, " ") !== false) {
                                return [$table, $field, "[%".ADFO_fn::clean_string($value_table). ' get="'.addslashes($value_field).'"]', $value_table,  $value_field];
                            } else {
                                return [$table, $field, "[%".ADFO_fn::clean_string($value_table).".".$value_field."]", $value_table,  $value_field];
                            }
                        } else if ($base_table == $value_table) {
                            if (strpos($value_field, " ") !== false) {
                                return [$table, $field, "[%".ADFO_fn::clean_string($table). ' get="'.addslashes($field).'"]', $table,  $field];
                            } else {
                                return [$value_table, $value_field, "[%".ADFO_fn::clean_string($table).".".$field."]", $table, $field];
                            }
                        }
                    }
                }
    ...
    private function  get_primary_key($table = "") {
        ...
        return ADFO_fn::get_primary_key($table);
    }
                ...
                switch ($f['op']) {
                    case "=":
                        ...
                        if (isset($f['table']) && substr($val,0,1) == '#') {
                            ...
                            $val = $wpdb->get_var($wpdb->prepare('SELECT `'.$column_name.'` FROM `'.$f['table'].'` WHERE `'. $f['table'] .'`.`'.$pri.'` = %s', substr($val,1)));
                        }
                        $array_where[] = $wpdb->prepare($f['column']." = %s", $val);
                        break;
                    case ">=":
                        $array_where[] = $wpdb->prepare($f['column']." >= %s", $f['value']);
                        break;
                    case ">=cast":
                        $array_where[] = $wpdb->prepare($f['column']." >= CAST(%s AS SIGNED)", $f['value']);
                        break;
                    case ">":
                        $array_where[] = $wpdb->prepare($f['column']." > %s", $f['value']);
                        break;
                    ...
                    case "<=":
                        ...
                        break;
                    case "<=cast":
                        $array_where[] = $wpdb->prepare($f['column']." <= CAST(%s AS UNSIGNED)", $f['value']);
                        break;
                    case "!=":
                        $array_where[] = $wpdb->prepare($f['column']." != %s", $f['value']);
                        break;
                    case "BETWEEN":
                    case "NOT BETWEEN":
                        $val = ADFO_fn::set_sql_where_between_value($f['column'], $f['value'], $f['op']);
                        if ($val != "") {
                            $array_where[] = $val;
                        }  
                        break;
                    case "LIKE%":
                        if (is_array($f['value']) || is_object($f['value'])) {
                            ...
                            foreach ($f['value'] as $val) {
                                $array_temp_where[] = $wpdb->prepare($f['column']." LIKE %s", $val."%");
                            }
                    ...
                    case "NOT LIKE":
                        ...
                        foreach ($f['value'] as $val) {
                            if (isset($f['table']) && substr($val,0,1) == '#') {
                                $sql = $wpdb->prepare('SELECT `'.ADFO_fn::sanitize_key($column_name).'` FROM `'.ADFO_fn::sanitize_key($f['table']).'` WHERE `'. ADFO_fn::sanitize_key($f['table']) .'`.`'.ADFO_fn::sanitize_key($pri).'` = %s', substr($val, 1));
                                $val = $wpdb->get_var($sql);
                            }
                            $array_temp_where[] = $f['column']." ".$op. ' "%'.$val.'%"';
                        }
                        $array_where[] = "(".implode("OR", $array_temp_where).")";
                        break;
                    ...
                    case "NOT IN":
                        if ( is_array($f['value'])) {
                            $value = $f['value'];
                        } else if (stripos($f['value'], '__#JSON#__') === false) {
                            if (is_string($f['value'])) {
                        ...
                        if (count ($new_ids) > 0 && strpos($f['column'],'`.`') !== -1) {
                            $array_or[] = "`" . ADFO_fn::sanitize_key($table) ."`.`". ADFO_fn::sanitize_key($pri) ."` ".$op." (".implode(", ",$new_ids).")";
                        }
                        if (count($array_or) == 1) {
    ...
    private function check_is_ok_for_draft($sql) {
        ...
        if (stripos($sql, 'DROP ') !== false && stripos($sql, 'DATABASE ') !== false)  {
            $this->last_error = __('You cannot remove the entire database','admin_form');
            return false;
        }
        ...
        if ((stripos($sql, 'ALTER ') !== false && stripos($sql, 'TABLE ') !== false) || (stripos($sql, 'DROP ') !== false && stripos($sql, 'TABLE ') !== false) || (stripos($sql, 'TRUNCATE ') !== false && stripos($sql, 'TABLE ') !== false)) {
            ...
            $info = ADFO_fn::get_dbp_option_table($alter_table);
            if ($info['status'] == 'DRAFT') {
                $is_ok_for_draft = true;
            } else {
                $this->last_error = __('Only tables in draft can be Altered. Go to structure and change the status of the table','admin_form');
            }
        }
    ...
    private function check_is_ok_for_close($sql) {
        ...
        $info = ADFO_fn::get_dbp_option_table(array_shift($temp_sql));
    ...
    private function get_list_check_same_column($list_last_get_col_info) {
        ...
        foreach ($list_last_get_col_info as $val ) {
            if (in_array($val->name, $temp_names)) {
                $this->last_error = sprintf(__('The query has multiple columns (<b>%s</b>) with the same name.', 'admin_form'), $val->name);
                $this->effected_row = 0;
                $this->items = [];
                 return false;
            }
