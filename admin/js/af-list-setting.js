...
function dbp_update_css_table() {
    let colors = ['blue', 'green', 'red','pink','yellow', 'purple', 'orange','gray', 'light', 'light-blue']
    ...
    for(x in colors) {
        ...
        jQuery('#dbp_preview_table_download').removeClass('dbp-download-button-'+colors[x]);
    }
    ...
    jQuery('#dbp_preview_table_download').addClass('dbp-download-button-'+color);
...
function dbp_update_column_sort() {
 ...
 /**
  * Aggiorno la preview se c'è la il bottone download
  */
 function dbp_update_download() {
    let val = jQuery('#dbp_table_export').val();
    if (val == "") {
        jQuery('#dbp_preview_table_download').css('display','none');
       
    } else {
        jQuery('#dbp_preview_table_download').css('display','block');
    }
}
