        ...
        dbp_edit.codemirror.on('keyup', function (cm, event) {
            ...
            setTimeout(function () {
                if (!cm.state.completionActive)
                    cm.showHint({ hint: wp.CodeMirror.hint.admin_form, completeSingle: 0 });
            }, 100);
        });
        if (size_height > 0) {
...
jQuery(document).ready(function ($) {
    ...
    wp.CodeMirror.registerHelper("hint", "admin_form", function (editor, options) {
    });
 ...
 */
 function af_columns_sql_query_edit() {
    ...
    if ( sql != "") {
        jQuery.ajax({
            ...
            url : ajaxurl,
            cache: false,
            data : {action:'af_columns_sql_query_edit',sql:sql},
...
/**
  ...
  */
function af_merge_sql_query_edit() {
    ...
    if (sql != "") {
        jQuery.ajax({
            ...
            url : ajaxurl,
            cache: false,
            data : {action:'af_merge_sql_query_edit',sql:sql},
            success: function(response) {
                dbp_close_sidebar_loading();
                if (!response.msg) {
                    jQuery('#dbp_dbp_title > .dbp-edit-btns').remove();
...
function get_fields_in_merge_sidebar(el_select) {
    ...
    jQuery.ajax({
        ...
        url : ajaxurl,
        cache: false,
        data : {action:'af_merge_sql_query_get_fields',table:jQuery(el_select).val()},
        success: function(response) {
...
function merge_sql_query_apply() {
    ...
    data.push({name: 'action', value: 'af_edit_sql_query_merge'});
...
function columns_sql_query_apply() {
    ...
    data.push({name: 'action', value: 'af_edit_sql_query_select'});
    data.push({name: 'section', value: 'table-browse'});
    dbp_open_sidebar_loading();
 ...
 * 
 */
function af_metadata_sql_query_edit() {
    ...
    if ( sql != "") {
        ...
        jQuery.ajax({
            ...
            url : ajaxurl,
            cache: false,
            data : {action:'af_metadata_sql_query_edit', sql:sql},
            success: function(response) {
                dbp_close_sidebar_loading();
                ...
                if (response.msg) {
                    ...
                    $field_select = jQuery('<select name="dbp_meta_table" id="dbp_meta_table" onchange="af_metadata_sql_query_edit_step2()"></select>');
                    ...
                    if (count_obj >= 1) {
                        ...
                        af_metadata_sql_query_edit_step2();
                    }
                }
            }
        });
 ...
 */
function af_metadata_sql_query_edit_step2() {
    sql = dbp_get_sql_string();
    if ( sql != "") {
        jQuery.ajax({
            type : "post",
            ...
            url : ajaxurl,
            cache: false,
            data : {action:'af_metadata_sql_query_edit_step2', table2:jQuery('#dbp_meta_table').val(), sql:sql},
            success: function(response) {
                let count_xxx = 0;
                ...
                $container.append('<div class="dbp-dropdown-line-flex dbp-line-choose-columns js-metadata-new-box"><label><span style="vertical-align:middle;margin-left:.5rem;"><input name="choose_meta['+count_xxx+']" type="checkbox" style="vertical-align:middle" value="" class="js-checkbox-metavalue-new" ></span><div style="width:250px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; display:inline-block; margin:.2rem .5rem; vertical-align:middle"><input class="js-metadata-new-name" onkeypress="new_input_metdata_update(this)" ></div></label> </div>');
        ...
        console.error('af_metadata_sql_query_edit_step2: sql is empty!')
    }
}
...
function dbp_addmeta_sql_query_apply() {
    sql = dbp_get_sql_string();
    if ( sql != "") {
        jQuery('.js-metadata-new-box').each(function() {
            let new_metadata = jQuery(this).find('.js-metadata-new-name').val();
            if (new_metadata != "") {
                jQuery(this).find('.js-checkbox-metavalue-new').val(new_metadata);
            }
        });
...
function new_input_metdata_update(el) {
    let new_metadata = jQuery(el).val();
    if (new_metadata != "") {
        jQuery(el).parents('.js-metadata-new-box').find('.js-checkbox-metavalue-new').prop( "checked", true );
    }
}
    ...
    if (sql.length > 7) {
        if (dbp_sql_check != sql) {
            jQuery.ajax({
                ...
                url : ajaxurl,
                cache: false,
                data : {action:'af_check_query', sql:sql},
                success: function(response) {
