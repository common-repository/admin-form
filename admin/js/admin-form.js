...
jQuery(document).ready(function ($) {
    ...
    function dropdown_filter_select_op(val, rif, clean = true) {
        ...
        if (val == "IN" || val == "NOT IN" || val == "Choose values") {
    ...
    // ORDINAMENTO DELLE COLONNE CON IL TRASCINAMENTO
    if (jQuery('.js-dragable-handle-browse-list').length > 0) {
        const tbody_list_to_drag = jQuery('.js-dragable-handle-browse-list').first().parents('tbody');
        tbody_list_to_drag.sortable({
            items: 'tr',
            opacity: 1,
            classes: {
            ...
            cursor: 'move',
            axis: 'y',
            handle: ".js-dragable-handle-browse-list",
            stop: function( event, ui ) {
                // trovo il valore minimo tra gli input jQuery('.js-dbp-order-value')
                let min = 99999999999;
                jQuery('.js-dbp-order-value').each(function() {
                    min = Math.min(this.value, min);
                });
                let update_data = [];
                jQuery('.js-dbp-order-value').each(function() {
                    this.value = min;
                    // trovo il valore del checkbox js-dbp-table-checkbox
                    let row_val = jQuery(this).parents('tr').find('.js-dbp-table-checkbox').val();
                    update_data.push({ids:dbp_tb_id[row_val], value : min})
                    min++;
                });
                // aggiorno in un'unica chiamata ajax i dati inviati con send_data
                let send_data = {update_data:update_data,action:'af_update_table_list_order_value', dbp_id:dbp_global_list_id, mysql_alias_name: jQuery(this).data('alias_name')};
                jQuery.ajax({
                    type : "post",
                    dataType : "json",
                    url : ajaxurl,
                    ...
                    success: function(response) {
                        ...
                        if (response.error != '') {
                            ...
                            jQuery('#dbp-alert-sql-error').empty();
                            jQuery('#dbp_cookie_msg').empty().html('Order saved').fadeIn();
                            setTimeout(()=>{ jQuery('.js-alert-snackbar').fadeOut();}, 3000);
                        }
                        jQuery('#'+response.divid).prop('disabled', false);
                    }
                });
    ...
    // GESTIONE DELLE COLONNE CON INPUT TEXT
    if (jQuery('.js-dbp-list-input-value').length > 0) {
        jQuery('.js-dbp-list-input-value').change(function() {
            let row_val = jQuery(this).parents('tr').find('.js-dbp-table-checkbox').val();
            if (jQuery(this).prop('disabled')) { return; }
            jQuery(this).prop('disabled', true);
            let send_data = {ids:dbp_tb_id[row_val], value : this.value,action:'af_update_table_list_input_value', dbp_id:dbp_global_list_id, mysql_alias_name: jQuery(this).data('alias_name'), divid: this.id};
            jQuery.ajax({
                type : "post",
                dataType : "json",
                url : ajaxurl,
                ...
                success: function(response) {
                    ...
                    if (response.error != '') {
                        ...
                        jQuery('.js-alert-snackbar').fadeOut();
                        jQuery('#dbp-alert-sql-error').empty();
                        jQuery('#dbp_cookie_msg').empty().html('Saved').fadeIn();
                        setTimeout(()=>{ jQuery('.js-alert-snackbar').fadeOut();}, 3000);
                    }
                    jQuery('#'+response.divid).prop('disabled', false);
                }
    ...
    // GESTIONE DELLE COLONNE CON CHECKBOX js-dbp-list-checkbox-value
    if (jQuery('.js-dbp-list-checkbox-value').length > 0) {
        jQuery('.js-dbp-list-checkbox-value').change(function() {
            let row_val = jQuery(this).parents('tr').find('.js-dbp-table-checkbox').val();
            if (jQuery(this).prop('disabled')) { return; }
            jQuery(this).prop('disabled', true);
            let send_data = {ids:dbp_tb_id[row_val], value : this.checked ? this.value : '',action:'af_update_table_list_input_value', dbp_id:dbp_global_list_id, mysql_alias_name: jQuery(this).data('alias_name'), divid: this.id};
            jQuery.ajax({
                type : "post",
                dataType : "json",
                url : ajaxurl,
...
function ajax_load_distinct_values(rif) {
    console.log ('ajax_load_distinct_values');
    ...
    if (typeof(memory_name) != 'undefined' && ((memory_name != '' && val_filter.substring(0, memory_name.length) == memory_name) || memory_name == '_#ALL#_')) {
        ...
        ajax_data = {'sql':sql, 'rif':rif, 'column':field, 'action': 'af_distinct_values', 'table': jQuery('#filter_search_orgtable_'+rif).val(),'filter_distinct':val_filter};
...
function dbp_ajax_detete_from_query(limit_start,total,filename) {
   ...
   // jQuery('.js-temp-checkbox').remove();
    ...
    jQuery.ajax({
        ...
        url : ajaxurl,
        cache: false,
        data : {tables:tables, sql:sql, action:'af_prepare_query_delete',limit_start:limit_start, total:total, dbp_filename:filename},
        success: function(response) {
 ...
 */
 function dbp_ajax_detete_from_query2(executed,total,filename) {
    jQuery.ajax({
        ...
        url : ajaxurl,
        cache: false,
        data : {action:'af_sql_query_delete',executed:executed, total:total, dbp_filename:filename},
        success: function(response) {
 ...
 * @param {*} json 
 */
function af_download_csv(sql, table_ids, limit_start, csv_filename) {
    data = {'sql':sql, 'ids':table_ids, 'limit_start':limit_start, 'csv_filename':csv_filename, 'action':'af_download_csv', 'section':'table-browse'};
    if (typeof dbp_global_list_id != 'undefined' && dbp_global_list_id > 0) {
        ...
        success: function(response) {
            ...
            if (response.error == "") {
                if (response.next_limit_start < response.count && response.next_limit_start > 0) {
                    ...
                    af_download_csv(sql, table_ids, response.next_limit_start, response.filename); 
                } else {
 ...
 */
function af_view_details(json) {
    let sql = jQuery('#sql_query_executed').val();
    dbp_open_sidebar_popup('view');
    data_to_send =  {'sql':sql, 'ids':json,'action':'af_view_details', 'section':'table-browse'};
    if (typeof dbp_global_list_id != 'undefined' && dbp_global_list_id > 0) {
 ...
 */
 function af_delete_confirm(ids, el) {
    if (jQuery('#dbp_dbp_title > .js-sidebar-btn').first().hasClass('js-btn-disabled')) return;
    let sql = jQuery('#sql_query_executed').val();
    jQuery(el).prop('onclick',null).off('click');
    jQuery(el).css({'color':'gray','cursor':'not-allowed'});
    opensidebar = dbp_open_sidebar_popup('delete');
    data_to_send = {'sql':sql, 'ids':ids,'action':'af_delete_confirm', 'section':'table-browse'};
    if (typeof dbp_global_list_id != 'undefined' &&  dbp_global_list_id > 0) {
        data_to_send.dbp_id = dbp_global_list_id;
    }
    jQuery('#dbp_dbp_title > .dbp-edit-btns').remove();
    ...
    jQuery.ajax({
        ...
        success: function(response) {
            ...
            if (opensidebar == "already_open") {
                ...
                if (response.count_rids) {
                    for (let crx = 0; crx < response.count_rids; crx++) {
                        jQuery('#dbp_form_deletes_rows').append('<span class="js-count-rids" style="display:none"></span>');
                    }
                }
            } else {
                let $form = jQuery('<div class="dbp-form-deletes-rows" id="dbp_form_deletes_rows"></div>');
                ...
                if (response.count_rids) {
                    for (let crx = 0; crx < response.count_rids; crx++) {
                        $form.append('<span class="js-count-rids" style="display:none"></span>');
                    }
                }
                jQuery('#dbp_dbp_content').append($form);
            ...
            if (response.show_msg != '') {
                ...
                response.show_msg = response.show_msg.replace("\%s", jQuery('.js-count-rids').length);
 ...
 * @param {array} ids {table.pri:val, ...}
 */
function af_trash(ids, el) {
    if (jQuery('#dbp_dbp_title > .js-sidebar-btn').first().hasClass('js-btn-disabled')) return;
    let el_id = jQuery(el).parents('tr').prop('id');
    data_to_send = {'dbp_id':dbp_global_list_id, 'ids':ids, 'el_id':el_id, 'action':'af_trash', 'section':'table-browse'};
    ...
    jQuery.ajax({
        ...
        data : data_to_send,
        success: function(response) {
            // se è success elimino la riga
            if (response.error) {
                alert(response.error);
            } else if (response.success) {
                if (jQuery('#subssubsub').length == 0) return;
                jQuery('#'+response.el_id).remove();
                if (response.count_executed > 0) {
                    count_executed = parseInt(response.count_executed);
                    let get_count = parseInt(jQuery('#dbp_post_count_trash').text());
                    get_count += count_executed;
                    if (get_count == 0) {
                        jQuery('#dbp_post_count_trash').parents('li').css('display', 'none');
                    } else{
                        jQuery('#dbp_post_count_trash').parents('li').css('display', 'inline-block');
                    }
                    jQuery('#dbp_post_count_trash').text(get_count);
                    let get_count2 = parseInt(jQuery('#dbp_post_count_all').text());
                    get_count2 -= count_executed;
                    jQuery('#dbp_post_count_all').text(get_count2);
                }
            } else {
                alert("I didn't remove any data.");
            }
        }
    });
}
 ...
 * @param {array} ids {table.pri:val, ...}
 */
function af_untrash(ids, el) {
    if (jQuery('#dbp_dbp_title > .js-sidebar-btn').first().hasClass('js-btn-disabled')) return;
    let el_id = jQuery(el).parents('tr').prop('id');
    data_to_send = {'dbp_id':dbp_global_list_id, 'ids':ids, 'el_id':el_id, 'action':'af_untrash', 'section':'table-browse'};
    ...
    jQuery.ajax({
        ...
        success: function(response) {
            ...
            if (response.error) {
                ...
                alert("I didn't restore any data.");
            }
        }
    });
}
 ...
 */
 function af_edit_details_v2(json, el) {
    let color_list = ['white', 'green','yellow','blue','red','purple', 'brown'];
    data_to_send = {'action':'af_edit_details_v2', 'section':'table-browse'};
...
function af_clone_details(json, el) {
    let color_list = ['white', 'green','yellow','blue','red','purple', 'brown'];
    data_to_send = {'action':'af_edit_details_v2', 'section':'table-browse', 'clone_record':'clone'};
    if (el instanceof HTMLElement) {
        data_to_send.div_id = jQuery(el).parents('tr').first().prop('id');
    }
    if (typeof dbp_global_list_id != 'undefined' &&  dbp_global_list_id > 0) {
...
function gp_form_btns_edit(btns_allow) {
    ...
    str_trash = (btns_allow.trash) ? ' <div class="dbp-submit-warning js-sidebar-btn" onclick="gp_trash_form_edit()">TRASH</div>' : '';
    return jQuery('<div class="dbp-edit-btns"><h3>EDIT CONTENT</h3>'+str_save+str_trash+str_delete+' <div id="dbp-bnt-edit-query" class="dbp-btn-cancel" onclick="dbp_close_sidebar_popup()">CANCEL</div></div>');
}
...
function gp_delete_form_edit() {
    ...
    af_delete_confirm([edit_ids]);
}

/**
 * trash dell record selezionato nell'edit
 */
function gp_trash_form_edit() {
    let edit_ids = jQuery('#dbp_edit_details').data('edit_ids');
    // trovo il valore del campo input div_id dentro la form con id dbp_edit_details
    let div_id = jQuery('#dbp_edit_details').find('input[name="div_id"]').val();
    let btn = jQuery('#'+div_id).find('.js-btn-trash');
    af_trash([edit_ids], btn);
...
function gp_submit_form() {
    ...
    $form.find('.js-dbp-validity').each(function() {
        ...
        if (jQuery(this).hasClass('js-dbp-autocoplete-id') && jQuery(this).val() == '0') {
            jQuery(this).val('');
        }
        if (!this.checkValidity()) {
            jQuery(this).addClass('js-dbp-add-show-error-border');
            if (jQuery(this).hasClass('js-dbp-autocoplete-id')) {
                const dfr = jQuery(this).parents('.js-dbp-form-row');
                dfr.find('.js-fake-autocomplete').css('display','none');
                dfr.find('.js-fake-autocomplete').addClass('dbp-input-error');
                dfr.find('.js-input-fake').empty();
                dfr.find('.js-dbp-autocoplete-id-title').css('display','block');
                dfr.find('.js-dbp-autocoplete-id-title').val('');
            } else {
    ...
    fn_dbp_autocomplete_multiple_values();
   
    //
    // var data = $form.serializeArray() ;
    var data = new FormData(document.getElementById('dbp_edit_details')); 
    data.append( 'action', 'af_save_details');
        ...
        success: function(response) {     
            ...
            if (response.error != "") {
                ...
                if ( response.msg != "") {
                    jQuery('.js-alert-snackbar').fadeOut();
                    jQuery('#dbp-alert-sql-error').empty();
                    jQuery('#dbp_cookie_msg').empty().html(response.msg).fadeIn();
...
/**
 * Prende i campi multipli e li raggruppa in un unico campo json e rimuove gli altri
 */
function fn_dbp_autocomplete_multiple_values() {

    // raggruppo campi multipli che hanno jquery.data['dbp-autocomplete-multiple'] uguale
    let multipli = [];
    jQuery('#dbp_edit_details input').each(function() {
        if (jQuery(this).data('dbp-autocomplete-multiple')) {
            if (!multipli.includes(jQuery(this).data('dbp-autocomplete-multiple'))) {
                multipli.push(jQuery(this).data('dbp-autocomplete-multiple'));
            }
        }
    });

    multipli.forEach(function(m) {
        let dbp_autocomplete_multiple = m;
        var dbp_autocomplete_multiple_values = [];
        console.log ("input[data-dbp-autocomplete-multiple= "+dbp_autocomplete_multiple);
        jQuery('input[data-dbp-autocomplete-multiple="'+dbp_autocomplete_multiple+'"]').each(function() {
            if (jQuery(this).val() != "" && jQuery(this).val() != "0" && jQuery(this).val() != "[]") {
                dbp_autocomplete_multiple_values.push(jQuery(this).val());
            }
        });
        // inserisco il valore nel primo campo e rimuovo gli altri
        jQuery('input[data-dbp-autocomplete-multiple="'+dbp_autocomplete_multiple+'"]').each(function() {
            if (this != jQuery('#dbp_edit_details input[data-dbp-autocomplete-multiple="'+dbp_autocomplete_multiple+'"]').get(0)) {
                jQuery(this).remove();
            }
        });
        if (dbp_autocomplete_multiple_values.length > 0) {
            jQuery('#dbp_edit_details input[data-dbp-autocomplete-multiple="'+dbp_autocomplete_multiple+'"]').first().val( JSON.stringify(dbp_autocomplete_multiple_values)); 
        } else {
            jQuery('#dbp_edit_details input[data-dbp-autocomplete-multiple="'+dbp_autocomplete_multiple+'"]').first().val(""); 
        }
    });
}
