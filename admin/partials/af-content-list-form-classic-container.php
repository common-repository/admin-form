...
<div class="dbp-lf-container-table js-lf-container-table js-dragable-table-box">
        ...
        <div class="js-dbp-lf-box-attributes" style="display:none">
            <div class="dbp-structure-grid">
                ...
                <div class="dbp-form-row-column js-row-allow-create-record">
                    <input type="hidden" class="js-prevent-exceeded-1000" value="<?php echo esc_attr($table_options->table_options); ?>" name="table_options[<?php echo esc_attr($key); ?>]">
                </div>
                ...
                <div class="dbp-form-row-column js-row-allow-create-record">
                    ...
                    <label class="dbp-label-grid dbp-css-mb-0"><span class="dbp-form-label"><?php _e('It allows you NOT to create the record','admin_form'); ?></span>
                        <?php ADFO_fn::html_select(['SHOW'=>'Yes', 'HIDE'=>'No'], true, 'name="table_allow_create['. esc_attr($key) .']"', $table_options->allow_create); ?>
                    </label>
                    <?php else: ?>
            ...
            <div class="dbp-form-row dbp-label-grid js-form-row-title">
                ...
                <input class="dbp-input js-prevent-exceeded-1000" style="width:100%"  name="table_title[<?php echo esc_attr($key); ?>]" value="<?php echo esc_attr($table_options->title); ?>">
            </div>

            <div class="dbp-form-row dbp-label-grid">
                <label><span class="dbp-form-label"><?php _e('Description','admin_form'); ?></span></label>
                <textarea class="dbp-input js-prevent-exceeded-1000" style="width:100%" rows="2" name="table_description[<?php echo esc_attr($key); ?>]"><?php echo esc_textarea($table_options->description); ?></textarea>
            </div>
        </div>
    </div>
    ...
    <?php if ($table_options->table_status == "DRAFT") : ?>
        <div class="adfo-new-field-box">
            <div class="button " onClick="dbp_duplicate_field(this)"><span class="dashicons dashicons-plus-alt"></span> <?php _e('New Field (Alter table)', 'admin_form'); ?></div>
            <?php ADFO_fn::echo_html_icon_help('dbp_list-list-form','new_field'); ?>
        </div>
        
    <?php else: ?>
        <div class="dbp-form-small-info js-form-small-info">
        <?php _e ('The table is in a published state and cannot be changed', 'admin_form'); ?>
        </div>
    <?php endif; ?>
</div>
