...
<?php if ($original_field_name != "" && $filter !== false) : ?>
    ...
    <div class="dbp-dropdown-line-flex">
        ...
        <span id="js_tf_select_label_<?php echo esc_attr($name_column); ?>" >Filter operators</span>
        <?php
        ADFO_fn::html_select($html_select_array, true, 'class="js-table-filter-select-op-partial"  id="js_tf_select_op_'.$name_column.'_1" data-rif="'.$name_column.'"', $def_op);
        ?>
    ...
    <div class="dbp-dropdown-line-flex" id="dbp_input_value2_box_<?php echo $name_column; ?>">
        <span class="dbp-filter-label"><?php _e('Value 2', 'admin_form'); ?></span>
        <?php if ($symple_type == "DATE") : ?>
    ...
    <div id="dbp_choose_values_box_<?php echo esc_attr($name_column); ?>">
        ...
        <?php
        ADFO_fn::html_select(['IN'=>'Choose values', 'NOT IN'=>'Exclude values'], true, ' class="js-table-filter-select-op-partial dbp-small-select-for-checkbox" id="js_tf_select_op_'.esc_attr($name_column).'_2" data-rif="'.esc_attr($name_column).'"', $def_op);
        ?>
        <div class="dbp-dropdown-de-select-click js-dropdown-select-all-checkboxes" data-rif="<?php echo esc_attr($name_column); ?>"><?php _e('Select all', 'admin_form'); ?></div>
        <div class="dbp-dropdown-de-select-click js-dropdown-deselect-all-checkboxes" data-rif="<?php echo esc_attr($name_column); ?>"><?php _e('Deselect all', 'admin_form'); ?></div>
        </div>
    ...
    <div class="dbp-dropdown-line dbp-dropdown-line-right">
        <div class="button dbp-btn-search js-dbp-btn-search"  data-rif="<?php echo esc_attr($name_column); ?>"><?php _e('OK', 'admin_form'); ?></div>
        <div class="button dbp-btn-search js-dbp-dropdown-btn-cancel"><?php _e('Cancel', 'admin_form'); ?></div>
    </div>
<?php endif;
